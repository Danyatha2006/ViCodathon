...
function App() {
  ...
  try {
    const response = await fetch(
      'http://127.0.0.1:8000/api/agent/process',
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          topic: 'New AI security research highlights risks in autonomous agents',
        }),
      }
    )
    ...
    console.log('AURA AI RESULT:', result)

    alert(
      `AI Analysis Complete\nDecision: ${
        result.status || result.decision || 'Received'
      }`
    )
  } catch (error) {
